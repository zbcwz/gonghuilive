<template>
  <div class="liveRoom" v-show="c_pageInited">
    <!-- <van-sticky> -->
    <div id="videoBox" ref="videoBoxref">
      <keep-alive>
        ...
        <ali-player v-once v-if="live.status == 1 && aliplayerOptions.source != ''" class="aliPlayer" :options="aliplayerOptions"
          ref="VueAliplayerV2"></ali-player>
        <!-- <img v-else style="width: 100%;height: 100%;" :src='live.cover'> -->
        <van-image v-else style="width: 100%;height: 100%;" :src='live.cover'>
      ...
      <div class="liveStatus" v-show="live.status != 1 ">
        <div class="live-status-sur" v-show="live.status >= 0">
          ...
          <p v-show="live.status != 2" class="liveStatus-time">
            ...
            <span v-else><a class="playback" v-show="live.playbackurl != ''" @click="viewPlayBack">观看直播回顾</a></span>
          </p>
        </div>
      </div>
    ...
    <van-tabs v-if="tablock" class="van-tabs-block" v-model="aselected">
          ...
          <div v-for="(textLiveValue,index) in message.textLiveList" class="comment" :key="index">
            ...
            <p :class="{
              ...
              <span class="commentImg" v-else-if="textLiveValue.type == 'image' || textLiveValue.type == 'timage'">
                ...
                <van-image class="commentImg" lazy-load @click="handleImgsClick(textLiveValue.text)" :src="textLiveValue.text"
                  @load="commentImgLoaded(this)">
                  <template v-slot:loading>
                    <van-loading type="spinner" size="20" />
                  </template>
                </van-image>
          ...
          <div v-for="(commentValue,index) in message.commentList" class="comment" :key="index">
            ...
            <p :class="{
              ...
              <span class="commentImg" v-else-if="commentValue.type == 'image'">
                ...
                <van-image class="commentImg" lazy-load @click="handleImgsClick(commentValue.text)" :src="commentValue.text">
                  <template v-slot:loading>
                    <van-loading type="spinner" size="20" />
                  </template>
                </van-image>
...
<script>
  ...
  export default {
    ...
    async created() {
      ...
      this.initQlgh();
      //console.log(this.initRoomInfo());
      await this.initRoomInfo();
      this.pageInited = true;
      this.tablock = true; //加载完毕后再显示
      this.connectWs();
      this.$nextTick(() => {//滚动条到底
    ...
    methods: {
      ...
      initRoomInfo: async function() { //页面信息、直播信息、分享的初始化
        var self = this;
        return new Promise((resolve, reject) => {

          console.log(self.roomClassifyList);

          var roomid = self.$route.params.id;
          ...
          //setTimeout(function(){
          $.ajax({
            url: "/app/index.php?i=" + self.pageconfig.uniacid +
              "&c=entry&m=ewei_shopv2&do=mobile&r=live.room.getRoomInfo",
            type: "GET",
            async: false,
            data: {
              id: roomid,
            ...
            success: function(d) {
              ...
              self.aliplayerOptions.cover = self.live.cover = d.result.list.room.cover != "" ? d.result.list
                .room.cover : d.result.list.room.thumb;
              self.live.playbackurl = d.result.list.room.playbackurl;
              self.live.reviewtype = d.result.list.room.reviewtype;
              ...
              var wxConfig = {
                ...
                desc: d.result.list.room.share_desc.length > 0 ? d.result.list.room.share_desc : self.pageconfig
                  .sharedesc,
                link: pagehost, // 分享链接
                imgUrl: d.result.list.room.share_icon.length > 0 ? d.result.list.room.share_icon : self.pageconfig
                  .shareimgUrl, // 分享图标
                success: function() {
                  // 用户确认分享后执行的回调函数
                },
              ...
              for (let i = 0; i < d.result.list.nestables.length; i++) {
                ...
                if (d.result.list.menu[d.result.list.nestables[i]].isshow == 0 || (d.result.list.nestables[
                      i] ==
      ...
      initQlgh: function() {
        ...
        this.modal.qlghuser = this.$store.state.qlghUser;
        console.log('aaaaaaaaa', this.modal.qlghuser);
      ...
      viewPlayBack: function() {

        if (this.live.reviewtype != 0) { //这里需要请求服务器获取播放地址


          this.aliplayerOptions.isLive = false;
          this.aliplayerOptions.skinLayout = this.dianskinLayout;
          this.aliplayerOptions.source = this.live.playbackurl;
          this.live.status = 1;

          /*   setTimeout(function(){
               alert(1);
               self.aliplayerOptions.isLive = false;
             },10000); */

        } else {
          window.location.href = this.live.playbackurl;
        }
